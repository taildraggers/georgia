"""Aggregates Georgia-located listings from every taildraggers.com
...
ones whose location ends in ", GA".
...
California/Colorado/Connecticut/Delaware/Florida repos' approach, with the
state filter swapped to "GA".
"""
...
PAGE_TITLE = "Taildragger Ads in Georgia"
STATE_ABBR = "GA"
...
_GEORGIA_LOCATION_RE = re.compile(r",\s*GA\s*$", re.IGNORECASE)


def _is_georgia(location: str) -> bool:
    return bool(_GEORGIA_LOCATION_RE.search(location.strip()))
...
def collect_listings() -> list[Listing]:
    ...
    for repo in SOURCE_SITES:
        ...
        georgia_rows = [row for row in rows if _is_georgia(row.location)]
        print(f"  [{repo}] {len(rows)} listings, {len(georgia_rows)} in Georgia")
        all_listings.extend(georgia_rows)
...
def render_html(listings: list[Listing]) -> str:
    ...
    rows_html = "".join(rows) if rows else (
        ...
        "No Georgia listings found in the latest run.</td></tr>"
    )
...
def main() -> None:
    ...
    print(f"[main] total unique Georgia listings: {len(listings)}")
