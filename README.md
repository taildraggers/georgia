...
Daily aggregator of Georgia-located taildragger classified listings,
...
repos' approach, with the state filter swapped to "GA".
...
- Each fetched page's listing table is parsed back into `(title, url,
  ...
  Location cell ends in `, GA` (case-insensitive), then de-duplicated by
...
- The result is rendered into `docs/index.html` titled **"Taildragger Ads
  in Georgia"**, using the exact same table/card layout, dark-mode
...
4. GitHub will publish the page at `https://taildraggers.github.io/georgia/`
...
```html
<iframe
  src="https://taildraggers.github.io/georgia/"
  title="Taildragger Ads in Georgia"
...
- This repo could be copied/adapted for other states by changing
  `STATE_ABBR` and `_GEORGIA_LOCATION_RE` in `main.py`, and updating
  `PAGE_TITLE`.
